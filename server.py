...
def run(clientsocket,mode):
    ...
    while(camera.isOpened()):
		...
		1.25, (0, 255, 0), 5)
                if  (preds[0] * 100>20) and exlabel != label:
                    exlabel=label
                    count=count+1
                    clientsocket.send(bytes(label,"utf-8"))
                    if mode==2 and count==2:
...
if __name__ == "__main__":
    ...
    #In order to be able to bind to the same port
    ...
    while True:
            ...
            if data == "recognise":
                mode=1
                k=run(clientsocket,mode)
                keypress = cv2.waitKey(1) & 0xFF
                # if the user pressed "q", then stop looping
                if keypress == ord("q") or k==-1:
                    break
            elif data == "calculate":
                mode=2
                print("calculate")
